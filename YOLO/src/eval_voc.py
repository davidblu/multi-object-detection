...
if __name__ == '__main__':
    ...
    #             nn.Linear(4096, 1470),
    #         )
    model.load_state_dict(torch.load('best_vgg16_bn.pth', map_location=lambda storage, loc: storage))
    model.eval()
    model.cuda()
    count = 0
    for image_path in tqdm(image_list):
        print(image_path)
        result = predict_gpu(model,image_path,root_path='VOC2007Test/') #result[[left_up,right_bottom,class_name,image_path],]
